<x-filament-panels::page>
    ...
    @php
        ...
        $groupedByMesin = collect($dataProduksi)->groupBy('mesin'); // key: "PRESS 1 - PAGI", dll
    @endphp
    ...
    <div class="space-y-12 mt-6">
        @forelse ($groupedByMesin as $mesinNama => $produksiList)
            @php
                ...
                $warna        = $selisih >= 0 ? 'text-green-400' : 'text-red-400';
                $tanda        = $selisih >= 0 ? '+' : '';
            ...
            <div class="bg-white dark:bg-zinc-900 rounded-sm shadow-lg border border-zinc-200 dark:border-zinc-700 overflow-hidden">
                <div class="bg-zinc-800 p-4 text-white">
                    <h2 class="text-lg font-bold text-center">
                        PEKERJA MESIN: {{ strtoupper($mesinNama) }} - {{ strtoupper($kodeUkuran) }}
                    </h2>
                </div>
                                ...
                                <tbody>
                                    @forelse ($pekerja as $i => $p)
                                        <tr class="{{ $i % 2 === 1 ? 'bg-zinc-50 dark:bg-zinc-800/50' : 'bg-white dark:bg-zinc-900' }} border-t border-zinc-300 dark:border-zinc-700">
                                            ...
                                            <td class="p-2 text-left text-xs border-r border-zinc-300 dark:border-zinc-700 text-zinc-900 dark:text-zinc-100 font-medium">
                                                {{ $p['nama'] ?? '-' }}
                                            </td>
                                            <td class="p-2 text-center text-xs border-r border-zinc-300 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300">
                                                {{ $p['jam_masuk'] ?? '-' }}
                                            ...
                                            <td class="p-2 text-right text-xs border-r border-zinc-300 dark:border-zinc-700 font-bold {{ $selisih < 0 ? 'text-red-600 dark:text-red-400' : 'text-zinc-700' }}">
                                ...
                                <tfoot class="bg-zinc-100 dark:bg-zinc-800 border-t-2 border-zinc-300 dark:border-zinc-600">
                                    <tr>
                                        <td colspan="7" class="p-3 text-center text-xs text-zinc-600 dark:text-zinc-400 space-x-3">
                                            ...
                                            <strong class="font-mono">{{ number_format($target) }}</strong>

                                            <span class="text-zinc-400">|</span>

                                            <span class="font-medium">Jam Produksi :</span>
                                            <strong class="font-mono">{{ number_format($jamKerja) }}</strong>
